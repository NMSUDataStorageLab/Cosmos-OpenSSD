...
{
	xil_printf("<====Entering ReqHandler====>\n");
	...
	while(1)
	{
		...
		{
			//shutdown handling
			FlushPageBuf(pageBufLpn, RAM_DISK_BASE_ADDR);
			PageMapFlushForOpenBlock();
			MetadataFlush();
		...
		{
			/*
			Print("CONFIG_SPACE_STATUS = 0x%x\n\r", 					Xil_In32(CONFIG_SPACE_STATUS));
			...
			{
//				Print the Current Sector and Requested Sector for write
				...
				//PmWriteD(&hostCmd, RAM_DISK_BASE_ADDR);  // 2nd write
			...
			{
//				Print the Current Sector and Requested Sector for Read
				xil_printf(">>>>>>read(%d, %d)<<<<<<\r\n", hostCmd.reqInfo.CurSect, hostCmd.reqInfo.ReqSect);
