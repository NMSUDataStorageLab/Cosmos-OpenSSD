...
void InitPageMap()
{
	DebugPrint("<====Entering InitPageMap====>\r\n");
	...
	for(i=0 ; i<DIE_NUM ; i++)
	{
		...
		{
			pageMap->pmEntry[i][j].ppn = 0xffffffff;  // 4294967295   =  4GB
			pageMap->pmEntry[i][j].valid = 1;
			pageMap->pmEntry[i][j].lpn = 0x7fffffff;  // 2147483647   =  2 GB
		}
	}

	xil_printf("[ ssd page map initialized. ]\r\n");
	DebugPrint("<====Leaving InitPageMap====>\r\n");

}
void InitBlockMap()
{
	DebugPrint("<====Entering InitBlockMap====>\r\n");
...
//This for loop is running continuously
	for (i = 0; i < BLOCK_NUM_PER_DIE; ++i)
		for (j = 0; j < DIE_NUM; ++j)
			if (!blockMap->bmEntry[j][i].bad && ((i != METADATA_BLOCK_PPN % DIE_NUM)|| (j != (METADATA_BLOCK_PPN / DIE_NUM) / PAGE_NUM_PER_BLOCK)))
			{
				// initial block erase
				//xil_printf("<====Loop for i & j : %d & %d====>\r\n",i,j);

				//xil_printf("<====WaitWayFree Called====>\r\n");

				WaitWayFree(j % CHANNEL_NUM, j / CHANNEL_NUM);

				//xil_printf("<====SsdErase Called after WaitWayFree====>\r\n");

				SsdErase(j % CHANNEL_NUM, j / CHANNEL_NUM, i);

				//xil_printf("<====SsdErase exited after WaitWayFree====>\r\n");

			}
	//DebugPrint("<====Final Loop for i & j : %d & %d====>\r\n",i,j);
	...
	DebugPrint("<====Leaving InitBlockMap====>\r\n");

}

void CheckBadBlock()
{
	DebugPrint("<====Entering CheckBadBlock====>\r\n");
	...
	while(loop > 0)
	{
		DebugPrint("<====SsdRead Called in CheckBadBlock loop====>\r\n");

		SsdRead(dieNo % CHANNEL_NUM, dieNo / CHANNEL_NUM, diePpn, tempBuffer);

		DebugPrint("<====waitWayFree Called in CheckBadBlock loop====>\r\n");
	...
	DebugPrint("<====Leaving CheckBadBlock====>\n");

}

int CountBits(u8 i)
{
	DebugPrint("<====Entering CountBits====>\n");
...
void InitDieBlock()
{
	DebugPrint("<====Entering InitDieBlock====>\n");
...
//	xil_printf("DIE_MAP_ADDR : %8x\r\n", DIE_MAP_ADDR);
	...
	DebugPrint("<====Leaving InitDieBlock====>\n");

}

void InitGcMap()
{
	DebugPrint("<====Entering InitGcMap====>\n");
...
//	xil_printf("GC_MAP_ADDR : %8x\r\n", GC_MAP_ADDR);
	...
	DebugPrint("<====Leaving InitGcMap====>\n");

}

void InitCiMap()
{
	DebugPrint("<====Entering InitCiMap====>\n");
...
{
	xil_printf("<====Entering FindFreePage====>\n");
	...
	if(blockMap->bmEntry[dieNo][dieBlock->dieEntry[dieNo].currentBlock].currentPage == (PAGE_NUM_PER_BLOCK-2)) // last page is a spare for pageMap of current block
	{
		...
		int i;
		for(i=dieBlock->dieEntry[dieNo].currentBlock ; i<(dieBlock->dieEntry[dieNo].currentBlock + BLOCK_NUM_PER_DIE) ; i++) // traverses all the blocks of the entire die and finds them from the beginning until you find an available block
		{
			if((blockMap->bmEntry[dieNo][i % BLOCK_NUM_PER_DIE].free) && (!blockMap->bmEntry[dieNo][i % BLOCK_NUM_PER_DIE].bad)) // if not a bad block then this free block can be used
			{
...
//				xil_printf("allocated free block: %4d at %d-%d\r\n", dieBlock->dieEntry[dieNo].currentBlock, dieNo % CHANNEL_NUM, dieNo / CHANNEL_NUM);
				...
				return dieBlock->dieEntry[dieNo].currentBlock * PAGE_NUM_PER_BLOCK; //Return a page no.
			}
		}

		dieBlock->dieEntry[dieNo].currentBlock = GarbageCollection(dieNo); // the entire die is not available after the block is marked garbage
...
//		xil_printf("allocated free block by GC: %4d at %d-%d\r\n", dieBlock->dieEntry[dieNo].currentBlock, dieNo % CHANNEL_NUM, dieNo / CHANNEL_NUM);
	...
	DebugPrint("<====Leaving FindFreePage====>\n");

}

int PrePmRead(P_HOST_CMD hostCmd, u32 bufferAddr)
{
	DebugPrint("<====Entering PrePmRead====>\n");
...
{
	DebugPrint("<====Entering PmWrite====>\r\n");

	u32 tempBuffer = bufferAddr;
	

	//DebugPrint("<====Current LPN Value= %d====>\r\n",lpn);
	DebugPrint(">>>>> CurSect before lpn calculation = 0x%x <<<<<<\n\r", hostCmd->reqInfo.CurSect);

	u32 lpn = hostCmd->reqInfo.CurSect / SECTOR_NUM_PER_PAGE;
	DebugPrint("<====Current LPN Value= %d====>\r\n",lpn);
	xil_printf("<====Current LPN Value= %d====>\r\n",lpn);
	...
	lpn++;
	xil_printf("<====Current LPN Value after addition= %d====>\r\n",lpn);
...
int PmWriteD(P_HOST_CMD hostCmd, u32 bufferAddr)
{
	DebugPrint("<====Entering PmWrite Double====>\r\n");

	u32 tempBuffer = bufferAddr;

	DebugPrint(">>>>> CurSect before Double wrtie lpn calculation = 0x%x <<<<<<\n\r", hostCmd->reqInfo.CurSect);


	u32 lpn = (hostCmd->reqInfo.CurSect / SECTOR_NUM_PER_PAGE);
	lpn= lpn+5;
	DebugPrint("<====Current Double LPN Value= %d====>\r\n",lpn);
	xil_printf("<====Current Double LPN Value= %d====>\r\n",lpn);
	...
	lpn++;
	xil_printf("<====Current Double LPN Value after addition= %d====>\r\n",lpn);
...
//		xil_printf("free page: %6d(%d, %d, %4d)\r\n", freePageNo, dieNo%CHANNEL_NUM, dieNo/CHANNEL_NUM, freePageNo/PAGE_NUM_PER_BLOCK);
	...
	DebugPrint("<====Leaving PmWrite Double====>\r\n");
...
{
	DebugPrint("<====Entering EraseBlock====>\n");
...
//	xil_printf("GC occurs!\r\n");
	...
	DebugPrint("<====Entering GarbageCollection====>\n");
	...
	DebugPrint("<====Leaving GarbageCollection====>\n");
...
{
	DebugPrint("<====Entering FlushPageBuf====>\n");

	if (lpn == 0xffffffff) // If in the beginning there is nothing within the page buffer, there is no need the flush
		return;

	u32 dieNo = lpn % DIE_NUM; // Calculation Die number
	u32 dieLpn = lpn / DIE_NUM; // Calculate lpn die in the first few lpn, can be understood as die0 is lpn0, lpn16 ... corresponding to dieLpn0, dieLpn1
	u32 ppn = pageMap->pmEntry[dieNo][dieLpn].ppn;
	u32 dieBuffer;

	if (ppn == 0xffffffff) // that the page cache has not yet written ppn
	{
...
//		xil_printf("free page: %6d(%d, %d, %4d)\r\n", freePageNo, dieNo%CHANNEL_NUM, dieNo/CHANNEL_NUM, freePageNo/PAGE_NUM_PER_BLOCK);
	...
	DebugPrint("<====Leaving FlushPageBuf====>\n");

}

void UpdateMetaForOverwrite(u32 lpn)
{
	DebugPrint("<====Entering UpdateMetaForOverwrite====>\n");
...
//		xil_printf("[unlink] dieNo = %d, invalidPageCnt= %d, diePbn= %d, blockMap.prevBlock= %d, blockMap.nextBlock= %d, gcMap.head= %d, gcMap.tail= %d\r\n", dieNo, blockMap->bmEntry[dieNo][diePbn].invalidPageCnt, diePbn, blockMap->bmEntry[dieNo][diePbn].prevBlock, blockMap->bmEntry[dieNo][diePbn].nextBlock, gcMap->gcEntry[dieNo][blockMap->bmEntry[dieNo][diePbn].invalidPageCnt].head, gcMap->gcEntry[dieNo][blockMap->bmEntry[dieNo][diePbn].invalidPageCnt].tail);
	...
	DebugPrint("<====Leaving UpdateMetaForOverwrite====>\n");
...
{
	DebugPrint("<====Entering PageMapFlushForCurrentBlock====>\n");
	...
	DebugPrint("<====Leaving PageMapFlushForCurrentBlock====>\n");

}

void PageMapFlushForOpenBlock() //save page map of open block
{
	xil_printf("<====Entering PageMapFlushForOpenBlock====>\n");
	...
	xil_printf("<====Leaving PageMapFlushForOpenBlock====>\n");

}

void MetadataFlush()
{
	xil_printf("<====Entering MetadataFlush====>\n");
	...
	xil_printf("<====Leaving MetadataFlush====>\n");

}

int CheckMetadata()
{
	xil_printf("<====Entering CheckMetadata====>\n");
	...
	if(*check4Byte != EMPTY_4BYTE)
		return 1;
	xil_printf("<====Leaving CheckMetadata====>\n");
...
{
	xil_printf("<====Entering RecoverMetadata====>\n");
	...
	xil_printf("<====Leaving RecoverMetadata====>\n");

}

void BadBlockTableBackup()
{
	xil_printf("<====Entering BadBlockTableBackup====>\n");
	...
	xil_printf("<====Leaving BadBlockTableBackup====>\n");

}

void RecoverPageMap()
{
	xil_printf("<====Entering RecoverPageMap====>\n");
