...
{
	DebugPrint("<====Entering GetRequestCMD====>\r\n");
	...
	*((u32*)(&hostCmd->reqInfo)) = Xil_In32((u32)(reqInfoAddr));
	...
	// Printing all the CMD STATUS
	DebugPrint("CmdStatus= 0x%x\n\r", hostCmd->CmdStatus);
	DebugPrint("ErrorStatus= 0x%x\n\r", hostCmd->ErrorStatus);
	DebugPrint("dScatterRegionLen= 0x%x\n\r", hostCmd->dScatterRegionLen);
	DebugPrint("dataTransferDirection= 0x%x\n\r", hostCmd->dataTransferDirection);
	...
	DebugPrint("UL Cmd = 0x%lu\n\r", (unsigned long)hostCmd->reqInfo.Cmd);
	//DebugPrint("Cmd = 0x%lu\n\r", (unsigned long)reqInfoAddr->reqInfo.Cmd);


	//Printing all the values of HOSTCMD
	//DebugPrint("Cmd = 0x%&x\n\r", hostCmd->reqInfo.Cmd);	//Test
	...
	DebugPrint("HostScatterNum = 0x%x\n\r\n\r", hostCmd->reqInfo.HostScatterNum);

	DebugPrint("<====Leaving GetRequestCMD====>\r\n");
...
{
	DebugPrint("<====Entering GetHostScatterRegion====>\r\n");
	...
	DebugPrint("<====Leaving GetHostScatterRegion====>\r\n");
...
{
	DebugPrint("<====Entering DmaDeviceToHost====>\r\n");
	u32 hostAddr;
	u32 flag;
...
void DmaHostToDevice(P_HOST_CMD hostCmd, u32 deviceAddr, u32 reqSize, u32 scatterLength)
{
	...
	DebugPrint("%x\n\r", Xil_In32(deviceAddr));
	DebugPrint("<====Leaving DmaHostToDevice====>\r\n");

}

void CompleteCmd(P_HOST_CMD hostCmd)
{
	DebugPrint("<====Entering CompleteCmd====>\r\n");
	u32 hostAddr, isDmaError;
	...
	DebugPrint("<====Leaving CompleteCmd====>\r\n");
